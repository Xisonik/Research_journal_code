...
class CLGRENV(gym.Env):
    ...
    def __init__(
        ...
        self.jetbot = self._my_world.scene.add(
            WheeledRobot(
                ...
                orientation=self.get_quaternion_from_euler(0),
            )
        )
    ...
    def get_gt_observations(self, previous_jetbot_position, previous_jetbot_orientation):
        ...
        delta_angle = math.degrees(abs(self.euler_from_quaternion(current_jetbot_orientation)[0] - quadrant*np.arccos(cos_angle)))
        ...
        observation = {
            ...
            "current_jetbot_orientation":math.degrees(self.euler_from_quaternion(current_jetbot_orientation)[0]),
    ...
    def reset(self, *, seed: Optional[int] = None, options: Optional[dict] = None):
        ...
        if self.event == 0:
            self.num_of_envs = np.random.choice([1,2,3,4,2,3,2,3])
        elif self.event == 1:
        ...
        self.jetbot.set_world_pose(new_pos ,self.get_quaternion_from_euler(new_angle))
        observations = self.get_observations()
    ...
    def get_position(self, x_goal, y_goal):
        ...
        while 1:
            ...
            if target_pos[1]>=-2.3 and target_pos[1]<=2.3 and target_pos[0]>=2.2 and ((target_pos[1]<=-0 and target_pos[0]>-target_pos[1]*0.572+2.35)
                        or (target_pos[1]>=0 and target_pos[0]>target_pos[1]*0.285+2.35)) and (np.abs(target_pos[1] - goal_world_position[1]) < track_width) and self.no_intersect_with_obstacles(target_pos[0:2],pos_obstacles):
                n = np.random.randint(2)
    ...
    def no_intersect_with_obstacles(self, target_pos,pos_obstacles):
        interect = True
        for pos_obstacle in pos_obstacles:
            if  np.abs(np.linalg.norm(pos_obstacle - target_pos)) < 0.6:
    ...
    def _on_contact_report_event(self, contact_headers, contact_data):
        ...
        for contact_header in contact_headers:
            ...
            pos_obstacles = np.array([[3.5,-0.8],[4,1]])
            if num_contact_data > 1 or not self.no_intersect_with_obstacles(self.current_jetbot_position,pos_obstacles): #1 contact with flore here yet
                self.collision = True
